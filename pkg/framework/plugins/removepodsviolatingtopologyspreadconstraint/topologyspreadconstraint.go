...
func New(args runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	removePodsViolatingTopologySpreadConstraintArgs, ok := args.(*componentconfig.RemovePodsViolatingTopologySpreadConstraintArgs)
	if !ok {
...
func (d *RemovePodsViolatingTopologySpreadConstraint) Deschedule(ctx context.Context, nodes []*v1.Node) *framework.Status {
	...
	// TODO should this be done on init
	labelSelector, err := metav1.LabelSelectorAsSelector(d.args.LabelSelector)
	if err != nil {
		return &framework.Status{
			Err: fmt.Errorf("failed to get label selectors from strategy's params: %w", err),
		}
	}
	...
	for _, namespace := range namespaces.Items {
		...
		for _, constraint := range namespaceTopologySpreadConstraints {
			...
			balanceDomains(client, d.handle.GetPodsAssignedToNodeFunc(), podsForEviction, constraint, constraintTopologies, sumPods, d.handle.Evictor().Filter, nodes)
		}
	}
	...
	for pod := range podsForEviction {
		...
		// TODO if a plugin doesn't import a specific pod evicter not sure how to hanld this?
		// Should really move this logic to happen the evicter anyways i dont know why a strategy is responsible for this.
